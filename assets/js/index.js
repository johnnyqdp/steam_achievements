...
function adicionaNome() {
    let a = "scrapper/usernameScrapper.php?username=" + username;
...
function pegarAchievements() {
    ...
    games.forEach( function (data, i) {
        ...
        let link = "scrapper/achievementScrapper.php?username=" + username + '&gameId=' + gameId + '&isMobile=' + isMobile;  
...
function getHtmlJogo (nomeClasse, appid, logo, name, horas) {
    ...
    return `<div onclick="clicarJogo('`+appid+`')" title="Ver conquistas adquiridas em `+name+`" class="` + nomeClasse + ` botaoAnimadinho" id="game_` + appid + `">
                <div `+ displayFlex + `>
                    ...
                    <div ` + classNegocinhos + `>               
                        <`+titleTag+` class="texto" style="margin-bottom:-20px">` + name + `</`+titleTag+`>
                        <br>
            ...
            ok:{
                ...
                callback: function() {
                    if (username && !jaClicou) {
                        ...
                        url = "scrapper/gameListScrapper.php?username=" + username;
